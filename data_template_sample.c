	...
	if ((EVENT_COUNTS > 0) && (eflag > 0)) {
		...
		for (i = 0; i < EVENT_COUNTS; i++) {
			if ((eflag & (1 << i)) & ALL_EVENTS_MASK) {
				...
				printf("print event origin  data---------------------------------:%s\n",sg_UpLoadMediaUrl_MediaUrl);
				event_count++; 
...
static int  getFileNames(char* pPathName,char* arrFileName[])
{
	int len=0;
	DIR *dir;
	struct dirent *ptr;
		...
		if( len>0)
		{
			...
			for(i=0;i<len;i++)
			{
				if(strstr(arrFileName[i],STRERROR)!=NULL)
				{
					...
					{
						// gaigai hardcore
						char* urlStr = readFirstLine(arrFileName[i]);
						// read file context
						if(urlStr!=NULL)
						{
